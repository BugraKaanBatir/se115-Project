...
public class Main {
    ...
    public static String bestWeekOfMonth(int month) {
        ...
        for(int i = 0; i<4; i++){
            int currentWeekTotalProfit = 0;
            int startDayIndex = i * 7;
            int endDayIndex = startDayIndex + 7;

            for(int j = startDayIndex; j<endDayIndex; j++){
                for( int k = 0; k<COMMS; k++){
                    currentWeekTotalProfit += profitData[j][month][k];
                }
            }

            if(currentWeekTotalProfit > maxProfit){
                maxProfit = currentWeekTotalProfit;
                bestWeek = i + 1;
            }
        }
